...
export default function Form({ topic, value, color }) {
  ...
  function save(num) {
    ...
    if (postImage) {
      ...
      const object = { [fileName]: {file: postImage, fecha: newDate.toString(), description: data.descriptionPost ? data.descriptionPost : '', enlace: data.enlacePost ? data.enlacePost : `${num}${newDate.getTime()}`, objectFit: data.objectPositionPost ? data.objectPositionPost : 'center', nota: '' } }
      setUserSuccess('Cargando')
  ...
  function handlerQuillValue(content, delta, source, editor) {
    // console.log(delta.ops[0].insert.image)

    delta &&  delta.ops[0].insert && delta.ops[0].insert.image ? setPostImage(delta.ops[0].insert.image): ''
    delta && delta.ops[0].insert && delta.ops[0].insert.image  ? setUrlPostImage(delta.ops[0].insert.image): ''
...
formats={formats} value={quillValue} onChange={handlerQuillValue} id={`${topic}-Post`} />
</div>
